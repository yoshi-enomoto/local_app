...
@section('content-body')
    ...
    <div class="box box-primary">
        <form class="form-horizontal" action="{{ route('hours.update', $hour) }}" method="POST" accept-charset="utf-8">
            @csrf
            @method('PUT')
            <div class="box-header with-border">
                ...
                <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                    ...
                    <div class="col-md-8">
                        <input class="form-control" type="date" name="date" value="{{ old('date', $hour->date->format('Y-m-d')) }}" min="2000-01-01" max="2999-12-31" required>
            ...
            <div class="box-body">
                ...
                {{-- @for($i=0; $i<10; $i++) --}}
                    <div class="form-group {{ $errors->has('hours.'.$i.'.category_id') || $errors->has('hours.'.$i.'.task_id') || $errors->has('hours.'.$i.'.hour') ? 'has-error' : '' }}">
                            ...
                            {{-- <select class="form-control parent-{{ $i }}" name="hours[{{ $i }}][category_id]"> --}}
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ old('hours.'.$i.'.category_id', $hour->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-3">
                            ...
                            {{-- <select class="form-control children-{{ $i }}" name="hours[{{ $i }}][task_id]" disabled> --}}
                                ...
                                @foreach($tasks as $task)
                                    <option value="{{ $task->id }}" data-val="{{ $task->category->id }}"
                                        {{ old('hours.'.$i.'.task_id', $hour->task_id) == $task->id ? 'selected' : '' }}>{{ $task->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-2">
                            <input class="form-control" type="number" name="hours[{{ $i }}][hour]" value="{{ old('hours.'.$i.'.hour', $hour->hour) }}" min="0" max="24" step="0.01" required>
...
{{--                         <input class="btn btn-sm btn-primary" type="submit" name="register" value="連続登録する">
            ...
            <div class="box-footer">
                <a href="javascript:history.back()" class="btn btn-sm btn-default">詳細へ戻る</a>
            </div>
        </form>
    </div>
@endsection
